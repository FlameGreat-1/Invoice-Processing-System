...
![Invoice Processing System Architecture](https://github.com/Flamegreat-1/invoice-processing-system/blob/main/docs/architecture.png)

Note: The architecture diagram shows the flow from client upload through processing to result delivery, including:
- Client interfaces with FastAPI App to upload files
- Processing through Gradio Interface and Hugging Face Space
- File Handler, OCR Engine, Data Extractor, Invoice Validator, and Invoice Exporter components
- External services like Google Cloud Vision and Tesseract OCR as fallbacks
- Celery Worker and Redis for task queue management
- Docker container running all components

For development purposes, here is the mermaid code to generate this diagram:
